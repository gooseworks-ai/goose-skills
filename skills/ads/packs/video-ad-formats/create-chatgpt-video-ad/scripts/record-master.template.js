...
// Look for the spec in the project root first, then in working/. Either
// location is canonical — the molecule's HOW_TO uses both conventions. This
// keeps a `working/thread.json` edit from silently no-op-ing when a stale
// root copy still exists, while still letting either layout work.
function resolveSpec(name) {
  const root = path.join(PROJECT_DIR, name);
  const working = path.join(PROJECT_DIR, 'working', name);
  if (fs.existsSync(root)) return root;
  if (fs.existsSync(working)) return working;
  console.error(`Missing ${name} — expected at ${root} or ${working}`);
  process.exit(1);
}
const THREAD_PATH = resolveSpec('thread.json');
const TIMELINE_PATH = resolveSpec('timeline.json');
...
// resolveSpec above already errored out if either file was missing.
