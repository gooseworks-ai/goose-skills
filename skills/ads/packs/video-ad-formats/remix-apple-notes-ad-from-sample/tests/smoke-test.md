# Smoke test — remix-apple-notes-ad-from-sample

**Goal:** a draft project + a remix-ready sample produce an approved note and a published MP4.

## Steps
1. Seed a draft project (`source_sample_id` of an Apple Notes sample with `recipe.note`,
   `format_key: "apple-notes"`) on a brand with `research_status: complete`.
2. Run the skill with the project id. Confirm the machine preflight passes (ffmpeg + Playwright +
   the `create-apple-notes-mockup` atom resolves).
3. Approve the drafted note when asked.

## Pass criteria
- The draft note keeps the source's pre-typed/typed paragraph counts (typed ±1) and typing pace,
