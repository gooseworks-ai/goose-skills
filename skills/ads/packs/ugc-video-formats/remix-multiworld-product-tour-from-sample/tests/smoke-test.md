...
Goal: prove `remix-multiworld-product-tour-from-sample` orchestrates a remix (dry, no render).

Steps:
1. Read `SKILL.md`.
2. Phase 0: given a `source_sample_id` with `recipe.format == "multiworld-product-tour"`, confirm the
...
5. Hand-off check: rendering calls `create-multiworld-product-tour-video-from-refs`, not a re-implementation.
