---
name: remix-multiworld-product-tour-from-sample
description: Remix a published "music-led multi-world product tour" ad from the Goose Ads library for a new brand — keep the source's format (a silent, music-led ~27s tour of 3 distinct 'third-place' worlds (one per product/scent), each a wide kinetic-calm arrival shot + a top-down macro product moment, closing on an HTML end card), swap in the brand's products/assets/copy, get the drafted scenes approved in-session, then render locally via create-multiworld-product-tour-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "music-led multi-world product tour" format tab calls.
---

# remix-multiworld-product-tour-from-sample

## Purpose

Given **one published music-led multi-world product tour sample** (from the Goose Ads library) + a **researched
brand**, produce a finished vertical ad for that brand. The source sample defines the
*shape* — a silent, music-led ~27s tour of 3 distinct 'third-place' worlds (one per product/scent), each a wide kinetic-calm arrival shot + a top-down macro product moment, closing on an HTML end card — and this skill swaps the *substance* into the brand's products,
...
[`create-multiworld-product-tour-video-from-refs`](../create-multiworld-product-tour-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows,
...
surfaces: Higgsfield Marketing Studio product_showcase clips (credits reserved server-side) + one nano_banana_2 end-card background + an ElevenLabs music bed; the FFmpeg trim/concat/mux is free.

## Inputs

| Input | Required | Notes |
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "multiworld-product-tour"` (stored as `recipe.format`). Without one, `create_ad_project { brand_id, name, source_sample_id }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`multiworld-product-tour`), `recipe` (the source's `config.json`: the full scene plan), `extracted_script`, `how_to`, `media_url` (watch it — this is a multi-scene format, so the shape matters). |
...
- [`create-multiworld-product-tour-video-from-refs`](../create-multiworld-product-tour-video-from-refs/SKILL.md) — the renderer (the sibling builder molecule):
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`multiworld-product-tour`),
...
`update_ad_project_script { project_id, script_drafts: { format: "multiworld-product-tour",
...
2. Hand off to `create-multiworld-product-tour-video-from-refs` to run the scene pipeline + assembly. Narrate with
...
- SEALED-BOTTLE safety gate on EVERY clip prompt (no open/cap-off/spray/smoke/vapor) — it caused a real P0 re-roll.
- Each product gets its OWN world palette + botanical companion; bottles are deliberately NOT color-coded — identity comes from the world, not the cap.
- Per-world two-shot grammar: a WIDE arrival (bottle stays small) → a top-down macro moment, no hands.
- Silent / music-led — no VO, no scene captions; text appears only on the end card.
- End-card text is HTML/Playwright-composited; nano_banana_2 renders the background ONLY (no typography).
