---
name: remix-ugc-product-from-sample
description: Remix a published Selfie product review / demo UGC video ad from the Goose Ads library for a new brand — keep the source's format, energy, and beat/monologue shape, swap in the brand's product, avatar archetype, and dialogue, get the reference stills + Seedance prompt approved in-session, then render ONE Seedance 2.0 reference-to-video call via create-ugc-product-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "Selfie product review / demo" UGC format tab calls.
---

# remix-ugc-product-from-sample

## Purpose

Given **one published Selfie product review / demo UGC sample** (from the Goose Ads library) + a **researched brand**,
produce a finished vertical UGC video for that brand. The source sample defines the *shape* —
format family (demo (beat-based, 3–4 internal cuts)), energy, dialogue cadence, and reference archetype — and this skill swaps
...
[`create-ugc-product-video-from-refs`](../create-ugc-product-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows, stage reporting,
...
only.** No captions. ≤ ~28 spoken words / 15s.

## Inputs

| Input | Required | Notes |
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "ugc-product"`. The paste-prompt carries it. Without one, `create_ad_project { brand_id, name, source_sample_id, format_key: "ugc-product" }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`ugc-product`), `remix_spec` (avatar/voice/world archetype + skills), `extracted_script` (the source dialogue), `how_to`, `media_url` (watch it if the shape is unclear). |
...
- [`create-ugc-product-video-from-refs`](../create-ugc-product-video-from-refs/SKILL.md) — the renderer (sibling builder molecule): normalizes refs
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`ugc-product`), `remix_spec`,
...
- **Product** → Required — the brand's product is held / shown across beats. Pull the real product image from `brand-assets/manifest.json`; a
...
- **Dialogue** → rewrite `extracted_script` in the brand's voice, honoring the word budget
  (≤ ~28 spoken words / 15s). Zero source-brand leakage (no source product, name, or code may survive).
- **Prompt** → author the four-block Seedance prompt via the builder's Phase 2 recipe. Format delta:
  Keep the source's 3–4 beat structure and POV pattern (front-cam beats carry dialogue + lip-sync; close-up/rear beats are POV, face off-screen). The product must appear in the beats that show it; restate its hero color features per beat.
...
`update_ad_project_script { project_id, script_drafts: { format: "ugc-product", ingredients: [...] },
...
1. `submit_render { project_id, kind: "full" }` (debits 1 credit at row creation — do this LAST,
   ...
   status: "running" }`.
2. Hand off to `create-ugc-product-video-from-refs` Phase 3: ONE `create-video-seedance-2-fal` call — refs in locked order, the approved
