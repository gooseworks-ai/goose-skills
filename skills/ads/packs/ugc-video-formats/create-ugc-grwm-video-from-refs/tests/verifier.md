...
Skill-specific pass criteria: `create-ugc-grwm-video-from-refs` must satisfy the
Output and Quality Checks in its `SKILL.md` — in particular:

- delivered mp4 is 9:16, within ±0.2s of the requested duration, **no burned
  captions**, native dialogue audio present;
- **single person throughout** — the review-loop confirmed no second body /
  partial body / stray hand / extra reflection, including the off-camera reaction
  beat and the mirror;
- **the full outfit is revealed** head-to-toe in at least one mirror/full-body
  beat;
- garment color/pattern/scale held across cuts (no morph, no invented text);
