...
Goal: prove `remix-value-prop-from-sample` orchestrates a remix (dry, no render).

Steps:
1. Read `SKILL.md`.
2. Phase 0: given a `source_sample_id` with `recipe.format == "value-prop"`, confirm the
...
5. Hand-off check: rendering calls `create-value-prop-video-from-refs`, not a re-implementation.
