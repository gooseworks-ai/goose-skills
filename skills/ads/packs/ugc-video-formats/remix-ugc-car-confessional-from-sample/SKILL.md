---
name: remix-ugc-car-confessional-from-sample
description: Remix a published Car-confessional monologue (yap) UGC video ad from the Goose Ads library for a new brand — keep the source's format, energy, and beat/monologue shape, swap in the brand's product, avatar archetype, and dialogue, get the reference stills + Seedance prompt approved in-session, then render ONE Seedance 2.0 reference-to-video call via create-ugc-car-confessional-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "Car-confessional monologue (yap)" UGC format tab calls.
---

# remix-ugc-car-confessional-from-sample

## Purpose

Given **one published Car-confessional monologue (yap) UGC sample** (from the Goose Ads library) + a **researched brand**,
...
[`create-ugc-car-confessional-video-from-refs`](../create-ugc-car-confessional-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows, stage reporting,
...
only.** No captions. ~35–45 words / 15s (calm ≈32–37, brisk ≈40–45).

## Inputs

| Input | Required | Notes |
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "ugc-car-confessional"`. The paste-prompt carries it. Without one, `create_ad_project { brand_id, name, source_sample_id, format_key: "ugc-car-confessional" }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`ugc-car-confessional`), `remix_spec` (avatar/voice/world archetype + skills), `extracted_script` (the source dialogue), `how_to`, `media_url` (watch it if the shape is unclear). |
...
- [`create-ugc-car-confessional-video-from-refs`](../create-ugc-car-confessional-video-from-refs/SKILL.md) — the renderer (sibling builder molecule): normalizes refs
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`ugc-car-confessional`), `remix_spec`,
...
- **Product** → Optional — testimonial by default; optional physical-product hold. SaaS/app served by the testimonial path (never fake on-screen UI). Pull the real product image from `brand-assets/manifest.json`; a
...
- **Dialogue** → rewrite `extracted_script` in the brand's voice, honoring the word budget
  (~35–45 words / 15s (calm ≈32–37, brisk ≈40–45)). Zero source-brand leakage (no source product, name, or code may survive).
- **Prompt** → author the four-block Seedance prompt via the builder's Phase 2 recipe. Format delta:
  ONE continuous take, no beats. Motion arc is prose, not a beat list; the static-body micro-motion rule matters more (no cuts to hide stillness). Parked-car confessional framing.
...
`update_ad_project_script { project_id, script_drafts: { format: "ugc-car-confessional", ingredients: [...] },
...
1. `submit_render { project_id, kind: "full" }` (debits 1 credit at row creation — do this LAST,
   ...
   status: "running" }`.
2. Hand off to `create-ugc-car-confessional-video-from-refs` Phase 3: ONE `create-video-seedance-2-fal` call — refs in locked order, the approved
