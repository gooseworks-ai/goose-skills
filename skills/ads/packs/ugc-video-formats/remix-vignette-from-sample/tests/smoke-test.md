...
Goal: prove `remix-vignette-from-sample` orchestrates a remix (dry, no render).

Steps:
1. Read `SKILL.md`.
2. Phase 0: given a `source_sample_id` with `recipe.format == "vignette"`, confirm the
...
5. Hand-off check: rendering calls `create-vignette-video-from-refs`, not a re-implementation.
