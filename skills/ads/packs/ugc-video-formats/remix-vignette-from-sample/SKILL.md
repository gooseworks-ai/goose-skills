...
Given **one published vignette ad (product cutouts over a kinetic background) sample** (from the Goose Ads library) + a **researched
brand**, produce a finished vertical ad for that brand. The source sample defines the
*shape* — a music-led ~10s vignette: brand product cutouts composited over a real-time kinetic background video (Pexels stock first, T2V fallback), with a cold-open text card, a product carousel, and an annotated specimen-sheet end card — and this skill swaps the *substance* into the brand's products,
...
[`create-vignette-video-from-refs`](../create-vignette-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows,
...
surfaces: the kinetic background — FREE when sourced from Pexels stock, paid only when it falls back to T2V (Veo/Kling/Seedance) — plus one ElevenLabs music bed; the birefnet cutouts, compositing, overlays, and mux are free.

## Inputs

| Input | Required | Notes |
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "vignette"` (stored as `recipe.format`). Without one, `create_ad_project { brand_id, name, source_sample_id }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`vignette`), `recipe` (the source's `config.json`: the full scene plan), `extracted_script`, `how_to`, `media_url` (watch it — this is a multi-scene format, so the shape matters). |
...
- [`create-vignette-video-from-refs`](../create-vignette-video-from-refs/SKILL.md) — the renderer (the sibling builder molecule):
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`vignette`),
...
`update_ad_project_script { project_id, script_drafts: { format: "vignette",
...
2. Hand off to `create-vignette-video-from-refs` to run the scene pipeline + assembly. Narrate with
...
- Pexels-first background, T2V ONLY on failure — this is the cost lever ($0.66 Pexels vs $8 T2V); skip Pexels after ~2 weak passes and fall back to Veo/Kling/Seedance.
- The background must be real-time kinetic, brand-themed, camera-locked — prompt 'NOT slow motion', use PHASE markers (never 'Shot N:', which forces cuts), derive the concept from the brand's ingredient/use-case.
- Palette-aware BG dim per variant: heavy desaturation on a bright/metallic BG, light on a naturally-contrasty one — desat is what stops the BG competing with the cutout.
- Cutouts: clean birefnet alpha (no halo/shadow), vertical-center (y=(H-h)/2, never bottom-anchor), width-anchored for 9:16 / height-anchored for 1:1.
- End card is an annotated specimen-sheet (EST year + brand + ingredient + claim), WHITE logo on a dark BG / cream on light — never a bare logo.
- Music-led, instrumental only (no VO/lyrics — they fight the on-screen text); loudnorm, then mux in a SEPARATE pass with explicit -map 0:v:0 -map 1:a:0.
