...
Goal: prove `remix-song-driven-music-video-from-sample` orchestrates a remix (dry, no render).

Steps:
1. Read `SKILL.md`.
2. Phase 0: given a `source_sample_id` with `recipe.format == "song-driven-music-video"`, confirm the
...
5. Hand-off check: rendering calls `create-song-driven-music-video-from-refs`, not a re-implementation.
