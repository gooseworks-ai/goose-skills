---
name: remix-ugc-grwm-from-sample
description: Remix a published Get Ready With Me / outfit try-on UGC video ad from the Goose Ads library for a new brand — keep the source's format, energy, and beat/monologue shape, swap in the brand's product, avatar archetype, and dialogue, get the reference stills + Seedance prompt approved in-session, then render ONE Seedance 2.0 reference-to-video call via create-ugc-grwm-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "Get Ready With Me / outfit try-on" UGC format tab calls.
---

# remix-ugc-grwm-from-sample

## Purpose

Given **one published Get Ready With Me / outfit try-on UGC sample** (from the Goose Ads library) + a **researched brand**,
produce a finished vertical UGC video for that brand. The source sample defines the *shape* —
format family (demo (beat-based, single-person off-camera reaction)), energy, dialogue cadence, and reference archetype — and this skill swaps
...
[`create-ugc-grwm-video-from-refs`](../create-ugc-grwm-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows, stage reporting,
...
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "ugc-grwm"`. The paste-prompt carries it. Without one, `create_ad_project { brand_id, name, source_sample_id, format_key: "ugc-grwm" }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`ugc-grwm`), `remix_spec` (avatar/voice/world archetype + skills), `extracted_script` (the source dialogue), `how_to`, `media_url` (watch it if the shape is unclear). |
...
- [`create-ugc-grwm-video-from-refs`](../create-ugc-grwm-video-from-refs/SKILL.md) — the renderer (sibling builder molecule): normalizes refs
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`ugc-grwm`), `remix_spec`,
...
- **Product** → Optional — an apparel/product piece being tried on or shown; SaaS served by the talking beats. Pull the real product image from `brand-assets/manifest.json`; a
...
- **Prompt** → author the four-block Seedance prompt via the builder's Phase 2 recipe. Format delta:
  Single person, GRWM framing; the reaction is off-camera (no second character). Keep the source's try-on / reveal beat order.
...
`update_ad_project_script { project_id, script_drafts: { format: "ugc-grwm", ingredients: [...] },
...
1. `submit_render { project_id, kind: "full" }` (debits 1 credit at row creation — do this LAST,
   ...
   status: "running" }`.
2. Hand off to `create-ugc-grwm-video-from-refs` Phase 3: ONE `create-video-seedance-2-fal` call — refs in locked order, the approved
