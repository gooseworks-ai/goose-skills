---
name: remix-ugc-walk-and-talk-from-sample
description: Remix a published Walk-and-talk selfie vlog (yap) UGC video ad from the Goose Ads library for a new brand — keep the source's format, energy, and beat/monologue shape, swap in the brand's product, avatar archetype, and dialogue, get the reference stills + Seedance prompt approved in-session, then render ONE Seedance 2.0 reference-to-video call via create-ugc-walk-and-talk-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "Walk-and-talk selfie vlog (yap)" UGC format tab calls.
---

# remix-ugc-walk-and-talk-from-sample

## Purpose

Given **one published Walk-and-talk selfie vlog (yap) UGC sample** (from the Goose Ads library) + a **researched brand**,
...
[`create-ugc-walk-and-talk-video-from-refs`](../create-ugc-walk-and-talk-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows, stage reporting,
...
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "ugc-walk-and-talk"`. The paste-prompt carries it. Without one, `create_ad_project { brand_id, name, source_sample_id, format_key: "ugc-walk-and-talk" }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`ugc-walk-and-talk`), `remix_spec` (avatar/voice/world archetype + skills), `extracted_script` (the source dialogue), `how_to`, `media_url` (watch it if the shape is unclear). |
...
- [`create-ugc-walk-and-talk-video-from-refs`](../create-ugc-walk-and-talk-video-from-refs/SKILL.md) — the renderer (sibling builder molecule): normalizes refs
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`ugc-walk-and-talk`), `remix_spec`,
...
- **Prompt** → author the four-block Seedance prompt via the builder's Phase 2 recipe. Format delta:
  ONE continuous selfie walk-and-talk take. Handheld walking motion; keep the subject front-cam the whole take. No internal cuts.
...
`update_ad_project_script { project_id, script_drafts: { format: "ugc-walk-and-talk", ingredients: [...] },
...
1. `submit_render { project_id, kind: "full" }` (debits 1 credit at row creation — do this LAST,
   ...
   status: "running" }`.
2. Hand off to `create-ugc-walk-and-talk-video-from-refs` Phase 3: ONE `create-video-seedance-2-fal` call — refs in locked order, the approved
