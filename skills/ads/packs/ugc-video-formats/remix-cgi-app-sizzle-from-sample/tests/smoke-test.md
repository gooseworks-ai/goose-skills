...
Goal: prove `remix-cgi-app-sizzle-from-sample` orchestrates a remix (dry, no render).

Steps:
1. Read `SKILL.md`.
2. Phase 0: given a `source_sample_id` with `recipe.format == "cgi-app-sizzle"`, confirm the
...
5. Hand-off check: rendering calls `create-cgi-app-sizzle-video-from-refs`, not a re-implementation.
