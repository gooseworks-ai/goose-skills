...
Given **one published premium 3D-CGI app-demo sizzle sample** (from the Goose Ads library) + a **researched
brand**, produce a finished vertical ad for that brand. The source sample defines the
*shape* — a premium ~22s 3D-CGI sizzle: a phone floats in a smoky-black studio and, across 6 feature beats, bursts REAL app-UI elements outward with rim-light + bokeh then settles, over VO + a cinematic music bed, closing on a brand end card — and this skill swaps the *substance* into the brand's products,
...
[`create-cgi-app-sizzle-video-from-refs`](../create-cgi-app-sizzle-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows,
...
surfaces: nano_banana_2 CGI plates + Kling 3.0 i2v (with a per-beat FFmpeg Ken-Burns fallback) + ElevenLabs VO + Music; PIL compositing, the audio mix, the 1.15x speed, and the grain pass are free.

## Inputs

| Input | Required | Notes |
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "cgi-app-sizzle"` (stored as `recipe.format`). Without one, `create_ad_project { brand_id, name, source_sample_id }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`cgi-app-sizzle`), `recipe` (the source's `config.json`: the full scene plan), `extracted_script`, `how_to`, `media_url` (watch it — this is a multi-scene format, so the shape matters). |
...
- [`create-cgi-app-sizzle-video-from-refs`](../create-cgi-app-sizzle-video-from-refs/SKILL.md) — the renderer (the sibling builder molecule):
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`cgi-app-sizzle`),
...
`update_ad_project_script { project_id, script_drafts: { format: "cgi-app-sizzle",
...
2. Hand off to `create-cgi-app-sizzle-video-from-refs` to run the scene pipeline + assembly. Narrate with
...
- REAL App Store screenshots + instructor faces are composited via PIL — NEVER AI-render UI or faces; the plate renders only the phone shell, studio, bokeh, and placeholder burst shapes.
- Ken-Burns is the per-beat fallback when Kling garbles the burst-out UI / distorts the phone — every beat carries a kenburns_fallback.
- VO-first timeline: render every VO cue, measure it, then derive beat windows from the measured durations (not word counts).
- Smoky-black (#0A0A0A) + amaranth (#E32652) rim + gold phone is the look; keep it consistent by anchoring plates on beat 1.
- Expose final_speed (source 1.15x) so the format's target duration scales.
