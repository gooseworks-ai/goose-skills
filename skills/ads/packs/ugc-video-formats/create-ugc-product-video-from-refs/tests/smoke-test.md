...
Goal: prove `create-ugc-product-video-from-refs` can perform its core workflow,
...
3. Author the four-block Seedance prompt (look directive → ref binding →
   consistency anchors → 3–4 beat shot list) for a 15s default render.
...
6. Save artifacts to `coworkers/test-runs/<timestamp>/create-ugc-product-video-from-refs/`.
...
no contact physics, per-ref job binding, distinct color naming), every paid call
is presented as a gate rather than fired, and the stitch dry-run resolves a valid
window. Missing details must be marked blocked, not silently skipped.
