---
name: remix-ugc-skincare-from-sample
description: Remix a published Bathroom skincare application demo UGC video ad from the Goose Ads library for a new brand — keep the source's format, energy, and beat/monologue shape, swap in the brand's product, avatar archetype, and dialogue, get the reference stills + Seedance prompt approved in-session, then render ONE Seedance 2.0 reference-to-video call via create-ugc-skincare-demo-video-from-refs and publish the MP4 back to Gooseworks with live stage reporting. This is what the app's "Bathroom skincare application demo" UGC format tab calls.
---

# remix-ugc-skincare-from-sample

## Purpose

Given **one published Bathroom skincare application demo UGC sample** (from the Goose Ads library) + a **researched brand**,
produce a finished vertical UGC video for that brand. The source sample defines the *shape* —
format family (demo (beat-based)), energy, dialogue cadence, and reference archetype — and this skill swaps
...
[`create-ugc-skincare-demo-video-from-refs`](../create-ugc-skincare-demo-video-from-refs/SKILL.md) and handles all Gooseworks I/O (render rows, stage reporting,
...
|---|---|---|
| `project_id` | yes (app flow) | A draft Gooseworks ad project with `source_sample_id` + `format_key: "ugc-skincare"`. The paste-prompt carries it. Without one, `create_ad_project { brand_id, name, source_sample_id, format_key: "ugc-skincare" }` first. |
| source sample | from project | `get_ad_template { template_id: <source_sample_id> }` → `format` (`ugc-skincare`), `remix_spec` (avatar/voice/world archetype + skills), `extracted_script` (the source dialogue), `how_to`, `media_url` (watch it if the shape is unclear). |
...
- [`create-ugc-skincare-demo-video-from-refs`](../create-ugc-skincare-demo-video-from-refs/SKILL.md) — the renderer (sibling builder molecule): normalizes refs
...
2. `get_ad_template { template_id: source_sample_id }` → keep `format` (`ugc-skincare`), `remix_spec`,
...
- **Product** → Required — the skincare product is applied / shown. Pull the real product image from `brand-assets/manifest.json`; a
...
- **Prompt** → author the four-block Seedance prompt via the builder's Phase 2 recipe. Format delta:
  Bathroom application demo with the no-phone / no-mirror constraint (never render a phone or mirror reflection). Keep the source's apply → glow beat order.
...
`update_ad_project_script { project_id, script_drafts: { format: "ugc-skincare", ingredients: [...] },
...
1. `submit_render { project_id, kind: "full" }` (debits 1 credit at row creation — do this LAST,
   ...
   status: "running" }`.
2. Hand off to `create-ugc-skincare-demo-video-from-refs` Phase 3: ONE `create-video-seedance-2-fal` call — refs in locked order, the approved
